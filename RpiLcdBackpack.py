...
class AdafruitLcd:
  ...
  def __init__(self):
    ...
    time.sleep(0.005)
    self.displaymode = self.__ENTRYLEFT | self.__ENTRYSHIFTDECREMENT
    self.writeFourBits(0x03)
    time.sleep(0.00015)
    self.writeFourBits(0x03)
  ...
  def home(self):
    self.writeCommand(self.__RETURNHOME)
    time.sleep(0.002)
  ...
  def setCursor(self, col, row):
    ...
    if row > 2: ##self.numlines:
      row = 1 ##self.numlines - 1  # we count rows starting w/0
    self.writeCommand(self.__SETDDRAMADDR | (col + self.row_offsets[row]))

  def displayLeft(self):
    """ These commands scroll the display without changing the RAM """
    self.writeCommand(self.__CURSORSHIFT | self.__DISPLAYMOVE | self.__MOVELEFT)

  def displayRight(self):
    """ These commands scroll the display without changing the RAM """
    self.writeCommand(self.__CURSORSHIFT | self.__DISPLAYMOVE | self.__MOVERIGHT)

  def autoscroll(self):
    """ This will 'right justify' text from the cursor """
    self.displaymode |= self.__ENTRYSHIFTINCREMENT
    self.writeCommand(self.__ENTRYMODESET | self.displaymode)
